...
const createNewGroupChat = async (req, res) => {
  ...
  if (finalUsers.length < 3) {
    throw new ExpressError(StatusCodes.BAD_REQUEST, 'A group must have at least 2 other members');
  }

  // Validate all users exist
  const foundUsers = await User.find({ _id: { $in: finalUsers } }).select('_id');

  if (foundUsers.length !== finalUsers.length) {
    throw new ExpressError(StatusCodes.BAD_REQUEST, 'One or more selected users do not exist');
  }
...
const updateGroupChatName = async (req, res) => {
  ...
  const chat = await Chat.findById(chatId).select('isGroupChat groupAdmin chatName');
  ...
  if (!chat.isGroupChat) {
    throw new ExpressError(StatusCodes.BAD_REQUEST, 'You can rename only group chats');
  }

  // validate admin
  if (chat.groupAdmin.toString() !== req.user.id) {
    throw new ExpressError(StatusCodes.FORBIDDEN, 'Only the group admin can rename the group');
  }

  // check if new name is same as old one
  const newName = chatName.trim();
  if (chat.chatName === newName) {
    throw new ExpressError(StatusCodes.BAD_REQUEST, 'New name must be different');
  }
...
const addUserToGroupChat = async (req, res) => {
  ...
  // verify chat
  const chat = await Chat.findById(chatId).select('isGroupChat groupAdmin users');
  if (!chat) {
    throw new ExpressError(StatusCodes.NOT_FOUND, 'Chat not found');
  }
  ...
  if (!chat.isGroupChat) {
    throw new ExpressError(StatusCodes.BAD_REQUEST, 'You can add users only in group chats');
  }

  // only admin can add users
  if (chat.groupAdmin.toString() !== req.user.id) {
    throw new ExpressError(StatusCodes.FORBIDDEN, 'Only the group admin can add users');
  }

  // check if user exists
  const userExists = await User.findById(userId).select('_id');
  if (!userExists) {
    throw new ExpressError(StatusCodes.NOT_FOUND, 'User does not exist');
  }

  // prevent adding same user again
  if (chat.users.map((u) => u.toString()).includes(userId)) {
    throw new ExpressError(StatusCodes.BAD_REQUEST, 'User is already in the group');
  }
...
const removeUserFromGroupChat = async (req, res) => {
  ...
  if (!chat.isGroupChat) {
    throw new ExpressError(StatusCodes.BAD_REQUEST, 'You can remove users only in group chats');
  }
  ...
  if (userId === adminId) {
    throw new ExpressError(StatusCodes.BAD_REQUEST, 'Group admin cannot be removed');
  }

  // ensure user is in group
  if (!chat.users.map((u) => u.toString()).includes(userId)) {
    throw new ExpressError(StatusCodes.BAD_REQUEST, 'User is not a member of this group');
  }
...
const updateGroupChatPicture = async (req, res) => {
  ...
  // fetch chat with minimal fields
  const chat = await Chat.findById(chatId).select('isGroupChat groupAdmin');
  ...
  if (!chat.isGroupChat) {
    throw new ExpressError(StatusCodes.BAD_REQUEST, 'You can update picture only for group chats');
  }
  ...
  // update & populate
  const updatedChat = await Chat.findByIdAndUpdate(chatId, { picture }, { new: true })
    .populate('users', '-password')
    .populate('groupAdmin', '-password');
